...
function ItemRow({ href, title, desc, chip, path, badge, onClick }: {
  ...
  return (
    <Link href={href as never} onClick={onClick}
      className="group flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 hover:shadow-[0_2px_12px_-4px_rgba(15,23,42,0.08)] transition-all">
...
export function Header() {
  ...
  /* Debounced open/close so moving across the gap between button → panel doesn't flicker */
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/97 backdrop-blur-md border-b border-gray-200/80 shadow-[0_1px_0_0_rgba(15,23,42,0.04)]">
      <div className="max-w-[82rem] mx-auto px-6 h-[68px] flex items-center gap-6">
        ...
        <nav className="hidden lg:flex items-center justify-center gap-0.5 flex-1" aria-label="Main navigation">
          ...
          {/* Solutions mega-menu */}
          <div className="relative" onMouseEnter={() => menuEnter('solutions')} onMouseLeave={menuLeave}>
            <button type="button"
              className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium text-gray-700 hover:text-brand-red rounded-xl hover:bg-gray-50 transition-all duration-150">
              {t('solutions')}
              <svg className={`w-3.5 h-3.5 text-gray-400 transition-transform duration-200 ${openMenu === 'solutions' ? 'rotate-180' : ''}`}
            ...
            <div
              ...
              className={`absolute left-0 top-full pt-2 w-[640px] z-50 transition-all duration-[180ms] ease-out ${dropCls('solutions')}`}
            >
              <div className="bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(15,23,42,0.20)] border border-gray-100/80 overflow-hidden flex">

                {/* Left sidebar — category tabs */}
                <div className="w-48 flex-shrink-0 bg-[#F7F8FA] p-3 space-y-1 border-r border-gray-100">
                  ...
                  {solutionsTab === 'supplier' && (
                    <div className="space-y-0.5">
                      {supplierItems.map((item) => (
                        <ItemRow key={item.href} {...item} onClick={() => setOpenMenu(null)} />
                      ))}
                    </div>
                  )}
          ...
          <div className="relative" onMouseEnter={() => menuEnter('technology')} onMouseLeave={menuLeave}>
            <button type="button"
              className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium text-gray-700 hover:text-brand-red rounded-xl hover:bg-gray-50 transition-all duration-150">
              {t('technology')}
              <svg className={`w-3.5 h-3.5 text-gray-400 transition-transform duration-200 ${openMenu === 'technology' ? 'rotate-180' : ''}`}
                viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clipRule="evenodd" />
              </svg>
            </button>
            ...
            <div
              ...
              onMouseLeave={menuLeave}
              className={`absolute left-0 top-full pt-2 w-[560px] z-50 transition-all duration-[180ms] ease-out ${dropCls('technology')}`}
            >
              <div className="bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(15,23,42,0.20)] border border-gray-100/80 overflow-hidden flex">
                  ...
                  {techTab === 'ops' && (
                    <div className="space-y-0.5">
                      {techOpsItems.map((item) => (
                        <ItemRow key={item.href} {...(item as Parameters<typeof ItemRow>[0])} onClick={() => setOpenMenu(null)} />
                      ))}
                    </div>
                  )}
                </div>
              ...
              return (
                <div key={section}>
                  <button type="button" onClick={() => toggleSection(section)}
                    ...
                    <svg className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${mobileSection === section ? 'rotate-180' : ''}`}
                      viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                      <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clipRule="evenodd" />
                    </svg>
                  </button>
